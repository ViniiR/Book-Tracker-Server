      ...
      if String.starts_with ~prefix:"Basic " auth then
        ...
        | Ok credentials -> (
            ...
            | [ username; password ] when is_valid username password ->
                handler req
            | _ -> respond ~status:`Bad_Request "Invalid credentials")
        | Error _ -> respond ~status:`Bad_Request "Invalid credentials"
...
let () =
  error_handler
  @@ with_pool Database.Pool.create
  @@ Dream.logger @@ authenticator
  @@ Dream.router
       [
         (*
